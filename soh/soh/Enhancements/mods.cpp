...
static u8 shinyableActorIds[53] = {
    ACTOR_EN_WALLMAS,
    ACTOR_EN_ZF,
    ACTOR_EN_YUKABYUN,
    ACTOR_EN_WF,
...
u8 CanBeShiny(Actor* actor) {
    u8 i;
    for (i = 0; i < ARRAY_COUNT(shinyableActorIds); i += 1) {
...
void RenderShines(Actor* actor) {
    Vec3f shinePos;
    s32 idx = (s32)(Rand_ZeroOne() * ARRAY_COUNT(shineSpots));
    if (gSaveContext.gameMode != GAMEMODE_NORMAL || gPlayState->pauseCtx.state != 0) {
        return;
    }
    if (Rand_ZeroOne() < 0.1f) {
        shinePos.x = actor->world.pos.x + shineSpots[idx].x;
        shinePos.y = actor->world.pos.y + shineSpots[idx].y;
        shinePos.z = actor->world.pos.z + shineSpots[idx].z;
        ...
        initParams.envColor.r = 235;
        initParams.envColor.g = 245;
        initParams.envColor.b = 255;
...
void SpawnShinyReward(Actor* actor) {
    ...
    Item_DropCollectible(gPlayState, &actor->world.pos, ITEM00_RUPEE_ORANGE);
}

void RegisterShiny() {
    GameInteractor::Instance->RegisterGameHook<GameInteractor::OnActorInit>([](void* refActor) {
        Actor* actor = static_cast<Actor*>(refActor);
        if (CVarGetInteger("gShinyEnabled", 0) && CanBeShiny(actor) && Rand_ZeroOne() < (1.0f / (s32)CVarGetInteger("gShinyChance", 8192))) {
    ...
    GameInteractor::Instance->RegisterGameHook<GameInteractor::OnEnemyDefeat>([](void* refActor) {
        Actor* actor = static_cast<Actor*>(refActor);
        if (CVarGetInteger("gShinyEnabled", 0) && actor->isShiny) {
