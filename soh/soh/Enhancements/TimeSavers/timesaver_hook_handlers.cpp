...
void EnFu_EndTeachSong(EnFu* enFu, PlayState* play) {
    if (Message_GetState(&gPlayState->msgCtx) == TEXT_STATE_CLOSING) {
        ...
        play->msgCtx.ocarinaMode = OCARINA_MODE_04;
        Flags::PLAYED_SONG_OF_STORMS_IN_WINDMILL.Set();
        Flags::LEARNED_SONG_OF_STORMS.Set();
        return;
    }
}
...
void EnDntDemo_JudgeSkipToReward(EnDntDemo* enDntDemo, PlayState* play) {
    ...
    switch (Player_GetMask(play)) {
        case PLAYER_MASK_SKULL: {
            Flags::OBTAINED_STICK_UPGRADE_FROM_STAGE.Set();
            return;
        }
        case PLAYER_MASK_TRUTH: {
            if (GameInteractor_Should(VB_DEKU_SCRUBS_REACT_TO_MASK_OF_TRUTH, true)) {
                Flags::OBTAINED_NUT_UPGRADE_FROM_STAGE.Set();
            }
            return;
        }
        default: {
    ...
    switch (id) {
        case VB_PLAY_TRANSITION_CS: {
            if (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipCutscene.LearnSong"), IS_RANDO) || IS_RANDO) {
                ...
                if ((gSaveContext.entranceIndex == ENTR_DESERT_COLOSSUS_OUTSIDE_TEMPLE) &&
                    !Flags::LEARNED_REQUIEM_OF_SPIRIT) {
                    Flags::LEARNED_REQUIEM_OF_SPIRIT.Set();
                ...
                u8 meetsBurningKakRequirements = LINK_IS_ADULT && gSaveContext.cutsceneIndex < 0xFFF0 &&
                                                 ...
                                                 Flags::USED_FOREST_TEMPLE_BLUE_WARP &&
                                                 Flags::USED_FIRE_TEMPLE_BLUE_WARP &&
                                                 Flags::USED_WATER_TEMPLE_BLUE_WARP &&
                                                 !Flags::BONGO_BONGO_ESCAPED_FROM_WELL;
                if (GameInteractor_Should(VB_BE_ELIGIBLE_FOR_NOCTURNE_OF_SHADOW, meetsBurningKakRequirements)) {
                    Flags::BONGO_BONGO_ESCAPED_FROM_WELL.Set();
                    // Normally happens in the cutscene
                    Flags::LEARNED_NOCTURNE_OF_SHADOW.Set();
            ...
            if (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipCutscene.Story"), IS_RANDO)) {
                ...
                if (LINK_IS_ADULT && (gEntranceTable[gSaveContext.entranceIndex].scene == SCENE_TEMPLE_OF_TIME) &&
                    ...
                    !Flags::RETURNED_TO_TEMPLE_OF_TIME_WITH_ALL_MEDALLIONS) {
                    Flags::RETURNED_TO_TEMPLE_OF_TIME_WITH_ALL_MEDALLIONS.Set();
                ...
                *should = false;
                ...
                switch (entranceIndex) {
                    ...
                    case ENTR_INSIDE_GANONS_CASTLE_7:
                        if (Flags::COMPLETED_FOREST_TRIAL &&
                            Flags::COMPLETED_WATER_TRIAL &&
                            Flags::COMPLETED_SHADOW_TRIAL &&
                            Flags::COMPLETED_FIRE_TRIAL && Flags::COMPLETED_LIGHT_TRIAL &&
                            Flags::COMPLETED_SPIRIT_TRIAL) {
                            Flags::DISPELLED_GANONS_TOWER_BARRIER.Set();
                        }
                        break;
                }
            }
        ...
        case VB_GORON_LINK_BE_SCARED: {
            if (ForcedDialogIsDisabled(FORCED_DIALOG_SKIP_NPC)) {
                ...
                Flags::STOPPED_GORON_LINKS_ROLLING.Set();
                *should = false;
            }
            break;
        }
        case VB_NOT_BE_GREETED_BY_SARIA:
            if (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipCutscene.Entrances"), IS_RANDO) &&
                !Flags::GREETED_BY_SARIA) {
                Flags::GREETED_BY_SARIA.Set();
                *should = true;
            }
            break;
        ...
        case VB_PLAY_PULL_MASTER_SWORD_CS:
            if (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipCutscene.Story"), IS_RANDO)) {
                if (!Flags::PULLED_MASTER_SWORD_FROM_PEDESTAL) {
                    // Normally, these would be done in the cutscene, but we're skipping it
                    Flags::PULLED_MASTER_SWORD_FROM_PEDESTAL.Set();
                    Flags::ENTERED_MASTER_SWORD_CHAMBER.Set();
                    Flags::SHEIK_SPAWNED_AT_MASTER_SWORD_PEDESTAL.Set();
                    Flags::TIME_TRAVELED_TO_ADULT.Set();
        ...
        case VB_OWL_INTERACTION: {
            if (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipOwlInteractions"), IS_RANDO) && *should) {
                ...
                if (((enOwl->actor.params & 0xFC0) >> 6) == 1) {
                    Flags::SPOKE_TO_KAEPORA_BY_LOST_WOODS.Set();
                }
        ...
        case VB_OVERRIDE_LINK_THE_GORON_DIALOGUE: {
            if (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipMiscInteractions"), IS_RANDO)) {
                ...
                if (!Flags::GORON_CITY_DOORS_UNLOCKED) {
                    *textId = 0x3036;
                    *should = true;
                }
            }
        ...
        case VB_DESPAWN_HORSE_RACE_COW: {
            if (Flags::WON_COW_IN_MALONS_RACE && CVarGetInteger(CVAR_ENHANCEMENT("CowOfTime"), 0)) {
                *should = false;
            }
            break;
        }
        case VB_PLAY_DRAIN_WELL_CS: {
            if (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipCutscene.Story"), IS_RANDO)) {
                *should = false;
                ...
                Flags::PLAYED_SONG_OF_STORMS_IN_WINDMILL.Set();
                Flags::DRAINED_WELL_IN_KAKARIKO.Set();
            }
            break;
        }
        ...
        case VB_PLAY_ROYAL_FAMILY_TOMB_EXPLODE: {
            if (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipMiscInteractions"), IS_RANDO)) {
                *should = Flags::DESTROYED_ROYAL_FAMILY_TOMB.Get();
            }
            break;
        }
            ...
            if (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipCutscene.Story"), IS_RANDO)) {
                *should = false;
                if (!Flags::RAINBOW_BRIDGE_BUILT) {
                    ...
                    Flags::RAINBOW_BRIDGE_BUILT.Set();
                }
            }
            break;
        }
...
void TimeSaverOnActorInitHandler(void* actorRef) {
    ...
    if (actor->id == ACTOR_EN_JJ) {
        enJjUpdateHook =
            GameInteractor::Instance->RegisterGameHook<GameInteractor::OnActorUpdate>([](void* innerActorRef) mutable {
                ...
                if (innerActor->id != ACTOR_EN_JJ || Flags::OFFERED_FISH_TO_JABU_JABU) {
                    return;
                }
            ...
            GameInteractor::Instance->RegisterGameHook<GameInteractor::OnActorUpdate>([](void* innerActorRef) mutable {
                ...
                if (!shouldKeepOpen) {
                    ...
                    shouldKeepOpen =
                        ...
                        (enhancement == 1 && Flags::OPENED_ZORAS_DOMAIN);
                }
    ...
    if (actor->id == ACTOR_EN_DU && gPlayState->sceneNum == SCENE_FIRE_TEMPLE) {
        if (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipCutscene.Story"), IS_RANDO) &&
            !CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipCutscene.GlitchAiding"), 0)) {
            Flags::SPOKE_TO_DARUNIA_IN_FIRE_TEMPLE.Set();
            Actor_Kill(actor);
        }
    }
...
void TimeSaverOnSceneInitHandler(int16_t sceneNum) {
    switch (sceneNum) {
        case SCENE_HYRULE_CASTLE:
            if (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipMiscInteractions"), IS_RANDO) &&
                !Flags::INF_ENTERED_HYRULE_CASTLE) {
                Flags::INF_ENTERED_HYRULE_CASTLE.Set();
                Flags::MET_CHILD_MALON_AT_CASTLE_OR_MARKET.Set();
                Flags::SPOKE_TO_CHILD_MALON_AT_CASTLE_OR_MARKET.Set();
            }
            break;
        case SCENE_LON_LON_RANCH:
            if (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipMiscInteractions"), IS_RANDO) &&
                GameInteractor_Should(VB_MALON_RETURN_FROM_CASTLE,
                                      Flags::TALON_RETURNED_FROM_CASTLE.Get()) &&
                (!IS_RANDO || !RAND_GET_OPTION(RSK_SHUFFLE_SPEAK) ||
                 Flags_GetRandomizerInf(RAND_INF_CAN_SPEAK_HYLIAN))) {
                Flags::SPOKE_TO_CHILD_MALON_AT_RANCH.Set();
                Flags::CHILD_MALON_SAID_EPONA_WAS_AFRAID_OF_YOU.Set();
                Flags::INVITED_TO_SING_WITH_CHILD_MALON.Set();
            }
            break;
        case SCENE_DEKU_TREE_BOSS:
            if (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipCutscene.BossIntro"), IS_RANDO)) {
                if (!Flags::BEGAN_GOHMA_BATTLE) {
                    Flags::BEGAN_GOHMA_BATTLE.Set();
                }
            }
            break;
        case SCENE_DODONGOS_CAVERN_BOSS:
            if (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipCutscene.BossIntro"), IS_RANDO)) {
                if (!Flags::BEGAN_KING_DODONGO_BATTLE) {
                    Flags::BEGAN_KING_DODONGO_BATTLE.Set();
                }
            }
            break;
        case SCENE_JABU_JABU_BOSS:
            if (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipCutscene.BossIntro"), IS_RANDO)) {
                if (!Flags::BEGAN_BARINA_BATTLE) {
                    Flags::BEGAN_BARINA_BATTLE.Set();
                }
            }
            break;
        case SCENE_FOREST_TEMPLE_BOSS:
            if (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipCutscene.BossIntro"), IS_RANDO)) {
                if (!Flags::BEGAN_PHANTOM_GANON_BATTLE) {
                    Flags::BEGAN_PHANTOM_GANON_BATTLE.Set();
                }
            }
            break;
        case SCENE_FIRE_TEMPLE_BOSS:
            if (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipCutscene.BossIntro"), IS_RANDO)) {
                if (!Flags::BEGAN_VOLVAGIA_BATTLE) {
                    Flags::BEGAN_VOLVAGIA_BATTLE.Set();
                }
            }
            break;
        case SCENE_WATER_TEMPLE_BOSS:
            if (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipCutscene.BossIntro"), IS_RANDO)) {
                if (!Flags::BEGAN_MORPHA_BATTLE) {
                    Flags::BEGAN_MORPHA_BATTLE.Set();
                }
            }
            break;
        case SCENE_SPIRIT_TEMPLE_BOSS:
            if (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipCutscene.BossIntro"), IS_RANDO)) {
                if (!Flags::BEGAN_TWINROVA_BATTLE) {
                    Flags::BEGAN_TWINROVA_BATTLE.Set();
                    Flags::BEGAN_NABOORU_BATTLE.Set();
                    Flags::NABOORU_ORDERED_TO_FIGHT_BY_TWINROVA.Set();
                }
            }
            break;
        case SCENE_SHADOW_TEMPLE_BOSS:
            if (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipCutscene.BossIntro"), IS_RANDO)) {
                if (!Flags::BEGAN_BONGO_BONGO_BATTLE) {
                    Flags::BEGAN_BONGO_BONGO_BATTLE.Set();
                }
            }
            break;
        case SCENE_GANONDORF_BOSS:
            if (CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipCutscene.BossIntro"), IS_RANDO)) {
                if (!Flags::BEGAN_GANONDORF_BATTLE) {
                    Flags::BEGAN_GANONDORF_BATTLE.Set();
                }
            }
            break;
    }
...
void TimeSaverOnFlagSetHandler(Flag flag) {
